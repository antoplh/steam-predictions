...
function processGameData() {
    if (window.gameFormData) {
        const { gamePrice, selectedLanguagesArray, selectedFeaturesArray, gameTagsArray } = window.gameFormData;
        ...
        performPCAOnNewData(gameTagsArray)
            .then(transformedTags => {
                ...
                const combinedArray = [parseFloat(gamePrice), F2P, ...selectedFeaturesArray, ...transformedTags, ...selectedLanguagesArray];
                    ...
                    .then(prediction => {
                        ...
                        fetch('models/centroids.json')
                            ...
                            .then(data => {
                                ...
                                displayResults(Math.ceil(prediction),cluster,transformedTags);
                            })
                            .catch(error => {
...
async function loadModelAndPredict(transformedData) {
    const { coefficients, intercept } = await loadModelCoefficients('models/model_coefficients.txt');
...
function displayResults(prediction, cluster) {
    ...
    resultsSection.innerHTML = `
        ...
        <div class="result-metric">
            ...
            <div class="quantile">is the prediction output for the amount of reviews your game might game on a platform like Steam. <strong>The amount of reviews for a game like this is in the <span style="color:${quantileColor};">${quantileText}</span></strong></div>
        </div>
